...
PROJECT_NAME="${VERCEL_PROJECT_NAME:-growthlabs-ai}"
VERCEL="npx --yes vercel@latest"

# Resolve the token's team (slug for --scope, id for the make-public API call).
# Empty for a personal-account token. Uses node (18+) with top-level await.
if [ -z "${VERCEL_SCOPE:-}" ] || [ -z "${VERCEL_TEAM_ID:-}" ]; then
  RESOLVED="$(VERCEL_TOKEN="$VERCEL_TOKEN" node --input-type=module -e '
...
if [ -n "${ADMIN_KEY:-}" ]; then ENV_ARGS+=(-e "ADMIN_KEY=$ADMIN_KEY"); fi
