import { neon } from "@neondatabase/serverless";

/**
 * Server-only handle to the team's database (Neon serverless Postgres over HTTP).
 * The connection string comes from `DATABASE_URL`, which the owner connects via
 * the database card and which is injected into the sandbox and passed to the live
 * host on publish. Resolved lazily (per call, not at module load) so the site
 * still builds and serves before a database is connected — the error only
 * surfaces if a query actually runs without `DATABASE_URL`.
 *
 * Use it only inside a `createServerFn()` handler or an `src/routes/api/*` route
 * (never client code):
 *
 *   const getPosts = createServerFn().handler(async () => {
 *     const rows = await sql()`select id, title, created_at from posts`;
 *     // Coerce non-primitive columns (timestamps are JS Dates) to strings before
 *     // returning to the client, or React will refuse to render them:
 *     return rows.map((r) => ({ ...r, created_at: String(r.created_at) }));
 *   });
 */
export const sql = () => {
  const url = process.env.DATABASE_URL;
  if (!url) {
    throw new Error(
      "DATABASE_URL is not set — connect a database (via the database card) before running queries.",
    );
  }
  return neon(url);
};

export interface LeadInput {
  name: string;
  email: string;
  company?: string;
  revenue_range?: string;
  message?: string;
  score?: string;
  source: "contact" | "scorecard" | "upload";
}

// Lazily create the leads table once per server instance; reset on failure so a
// transient DB error doesn't poison every later request.
let tableReady: Promise<unknown> | null = null;
const ensureLeadsTable = () => {
  if (!tableReady) {
    tableReady = sql()`
      CREATE TABLE IF NOT EXISTS leads (
        id SERIAL PRIMARY KEY,
        name TEXT NOT NULL DEFAULT '',
        email TEXT NOT NULL DEFAULT '',
        company TEXT DEFAULT '',
        revenue_range TEXT DEFAULT '',
        message TEXT DEFAULT '',
        score TEXT DEFAULT '',
        source TEXT DEFAULT '',
        created_at TIMESTAMPTZ NOT NULL DEFAULT now()
      )
    `.catch((e) => {
      tableReady = null;
      throw e;
    });
  }
  return tableReady;
};

export async function insertLead(lead: LeadInput): Promise<void> {
  await ensureLeadsTable();
  await sql()`
    INSERT INTO leads (name, email, company, revenue_range, message, score, source)
    VALUES (${lead.name}, ${lead.email}, ${lead.company ?? ""}, ${lead.revenue_range ?? ""},
            ${lead.message ?? ""}, ${lead.score ?? ""}, ${lead.source})
  `;
}

export async function getRecentLeads(limit = 100) {
  await ensureLeadsTable();
  const rows = await sql()`
    SELECT id, name, email, company, revenue_range, source, score, created_at
    FROM leads ORDER BY created_at DESC LIMIT ${limit}
  `;
  return rows.map((r) => ({ ...r, created_at: String(r.created_at) }));
}
